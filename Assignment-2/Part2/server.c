...
void *process_request(void *args)
{
    ...
    pthread_mutex_unlock(&print_unlock);
...
void *send_to_lb(void *args)
{
    ...
    char *msg = "Hey, I have got a free thread";
    ...
    while(1)
    {
        ...
        sendto(send_sockfd, (const char*)msg, strlen(msg) + 1, 0, (const struct sockaddr *) &lbaddr, sizeof(lbaddr));
        pthread_mutex_lock(&print_lock);
...
void *recv_handler(void *args)
{
   ...
   while(1)
   {
        ...
        int recv_val = 0;
        len = sizeof(cliaddr);
        recvfrom(sockfd, &recv_val, sizeof(recv_val), 0, (struct sockaddr*)(cliaddr), sizeof(cliaddr));
        pthread_mutex_lock(&print_lock);
        printf("A new request received at recv thread with value: %d", recv_val);
        pthread_mutex_unlock(&print_lock);
